...
async function registerUser(worker, email = "payer@example.com") {
  ...
  return { cookie: `guanchen_session=${session}`, json: await response.json() };
}
...
test("guests have zero credits and must log in before unlocking paid reports", async () => {
  ...
  assert.deepEqual(await creditsResponse.json(), { authenticated: false, credits: 0, newUserGift: 5, message: "登录后新用户可免费获得 5 积分，用于解锁八字或紫微斗数完整报告。" });
